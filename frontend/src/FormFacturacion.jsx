...
const FormFacturacion = () => {
  ...
  const historialFiltrado = useMemo(
    () => historial.filter((item) => (item?.orden?.motocicleta_placa || '').toLowerCase().includes(busquedaPlaca.trim().toLowerCase())),
    [historial, busquedaPlaca]
  );
  ...
  const eliminarFactura = async (item) => {
    const facturaId = item?.factura?.codigo;
    if (!facturaId) return;
    if (!window.confirm(`¿Eliminar la factura #${facturaId}? La orden #${item?.orden?.codigo} volverá a estado Finalizado.`)) return;
    setError('');
    try {
      ...
      setError('Error de conexión eliminando la factura.');
    }
  };
  ...
  return (
    <div className="app-container facturacion-page">
      ...
      <div className="bitacora-panel facturacion-history-panel">
        ...
        <div className="facturacion-search" style={{ marginBottom: '12px' }}>
          <input
            type="text"
            value={busquedaPlaca}
                  ...
                  <tr key={`${item?.orden?.codigo || 'orden'}-${item?.factura?.codigo || 'factura'}`}>
                    ...
                    <td>
                      {item?.factura?.metodo_pago === 'PayPal' ? (
                        ...
                        >
                          {item.factura.comprobante_pago.split('/').pop()}
                        </a>
                      ) : '-'}
                    </td>
                    <td>
